...
export class EditProvidersComponent implements OnInit {
  ...
  async getData(ID:any){
    ...
    await(this.dbService.GetSpecificProvider(ID).subscribe((ret:any) => {
      if(ret != "false"){
        ...
        this.BoxSize = Number(a[1]);
        this.BatchSize = Number(a[2]);
  ...
  async Delete(){
    await(this.dbService.DeleteProvider(this.ProviderName).subscribe((ret:any) => {
      if(!(String(ret).includes("Error"))){
        ...
        this.DisplayErrormessage = "The Provider has been deleted";
        let element: HTMLButtonElement = document.getElementById('ErrorButton') as HTMLButtonElement;
        element.click();
        this.Back();
      }else{
  ...
  async Save(){
    ...
    if(this.BoxSize > 300 || this.BatchSize > 150){
      ...
      await(this.dbService.EditProvider(this.oldProviderName, this.ProviderName, String(this.BoxSize), String(this.BatchSize)).subscribe((ret:any) => {
        if(!(String(ret).includes("Error"))){
          this.PopupTitle = "Success"
          this.DisplayErrormessage = "The Provider has been editted";
          let element: HTMLButtonElement = document.getElementById('ErrorButton') as HTMLButtonElement;
          element.click();
          this.Back();
        }else{
